# Crewculator — Claude Code Agents & Skills
...
**When to invoke:**
- Before adding a new library or dependency (rare in this project — we have ~0 runtime deps)
- When a task requires a pattern we haven't used yet
...
**Output:** Diff report with proposed actions; user approves before any file changes.
...
**Not used** — type-gated to `webapp` projects per `.claude/type-manifest.yaml`. Crewculator is a `tool` project and does not adopt this agent. Visual review for the single page is done manually by the operator on the deployed Netlify preview.
...
**What it does:**
1. Runs a build check (in this project, that's `node --test` for any pure JS already written, or just confirming `netlify dev` doesn't blow up)
...
### /kill-this — End Task (Per-Task PR + Log Append)
...
**What it does:**
1. Runs the relevant tests for what changed (unit + smoke if UI was touched)
...
### /its-dead — End Session (Once Per Claude Window)
...
**Per-session files:** `sessions/YYYY-MM-DD-HHMM-<dev>-<slug>.md` on the orphan `sessions` branch via `.sessions-worktree/`. `<dev>` comes from `~/.claude/devname`. The active JSONL transcript path is captured in the file's frontmatter for `/read-the-tape` audits.

**Task model:** `PROJECT_PLAN.md` is a phase-boundary document — read at planning, written at retro. Current-phase tasks materialize as GitHub Issues with `phase:N` + `points:X` labels. The plan stays untouched mid-phase.
